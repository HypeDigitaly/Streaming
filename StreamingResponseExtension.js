export const StreamingResponseExtension = {
  ...
  render: async ({ trace, element }) => {
    ...
    function addAIInfoFooter(attemptedModels) {
      ...
      // Check if any models timed out
      const timeoutOccurred = attemptedModels.some((m) => m.timedOut === true);
      ...
      const languageMessages = {
        ...
        de: {
          ...
          timeout: "Zeitüberschreitung aufgetreten.",
        },
        uk: {
          success: "Відповідь згенерована ШІ.",
          failure: "Генерація ШІ не вдалася.",
          timeout: "Виникла помилка часу очікування.",
        },
      };
      ...
      if (wasSuccess) {
        ...
        aiInfoText.textContent = messages.timeout;
      ...
      if (attemptedModels.length > 0) {
        tooltipHTML += attemptedModels
          .map((attempt) => {
            ...
            const timeoutIndicator = attempt.timedOut === true ? tooltipText.timeout : "";
            return `${statusIcon} ${displayName}${timeoutIndicator}`;
          })
      ...
      try {
        ...
        try {
          ...
          if (error.name === "AbortError") {
            if (payload.debugMode === 1) {
              console.warn(`Timeout occurred for ${endpoint}`);
            }
    ...
    async function orchestrateLLMCalls(trace) {
      ...
      for (const modelId of modelSequence) {
        ...
        const currentAttempt = { id: model.id, success: null, timedOut: false };
        ...
        currentAttempt.timedOut = !success; // Assuming failure means timeout here
